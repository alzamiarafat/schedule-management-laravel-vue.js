                                ...
                                <div class="card-body">
                                    <form @submit.prevent="userUpdate">
                                        <div class="row">
                                            <div class="col-6">
                                                <label>Name</label>
                                                <input class="form-control" type="text" v-model="store.name" name="name" placeholder="Enter Name">
                                        ...
                                        <div class="row">
                                            <div class="col-6">
                                                ...
                                                <input tyepe="password" class="form-control" name="password" v-model="store.password" placeholder="Enter Password" required>
                                        ...
                                        <div class="row">
                                            <div class="col-6">
                                                ...
                                                <select class="form-control" v-model="store.role">
                                                    ...
                                                    <option v-for="role in roles" :key="role.id" :vaue="role.id">{{ role.name }}</option>
                                                </select>
                                                <!-- <p class="text-danger" v-text="errors.phoneNumberError"></p> -->
                                            </div>
                                        </div>
                                                ...
                                                <div class="row" v-for="permission in permissions" :key="permission.id">
                                                    <div class="col-12">
                                                        ...
                                                        <div class="form-check">
                                                            <input class="form-check-input" type="checkbox" v-model="store.permissions" :value="permission.id" name="permissionse" :id="permission.id">
                                                            <input class="form-check-input" type="checkbox" v-model="store.permissions" :value="permission.id" :id="permission.id" name="permissionse">
...
<script>
    ...
    export default {
        ...
        methods : {
            ...
            getUser(){
                axios.post('/api/user/edit', { token : this.$store.state.token, id: this.userId } ).then((response) => {
                    this.store = response.data.result;
                    for(var i=0; i<response.data.permissions.length; i++){
                        this.store.permissions[i] = JSON.stringify(response.data.permissions[i]['id'])
                    }
                    this.store.role = this.store.roles[0]['name'];
                })
            },

            userUpdate() {
                axios.post('/api/user/update', { token : this.$store.state.token , data: this.store} ).then((response) => {
                    this.$swal({
                        title: "Success",
